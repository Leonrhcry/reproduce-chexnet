...
class CXRDataset(Dataset):
    def __init__(self, merged_csv, img_dir, transform=None):
        """
        Args:
            merged_csv (str): Path to the merged CSV file containing image filenames and labels.
            img_dir (str): Directory containing the images.
            transform (callable, optional): Optional transform to be applied on a sample.
        """
        # Load the merged CSV file
        self.df = pd.read_csv(merged_csv, encoding='ISO-8859-1')
        self.transform = transform
        self.path_to_images = img_dir  # Path to the images folder

        # Labels corresponding to conditions in the dataset
        self.PRED_LABEL = [
    ...
    def __getitem__(self, idx):
        # Get the image filename (Image Index should be a string in the CSV)
        ...
        # Check if the image exists in the starter_images directory
        ...
        # Convert the label to float32 (required by BCELoss)
...
def evaluate_model(model, dataloader):
    ...
    f1 = f1_score(all_labels, (all_predictions > 0.5).astype(int), average='macro')
