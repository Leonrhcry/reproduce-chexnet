...
def evaluate_model(model, dataloader):
    ...
    f1 = f1_score(all_labels, (all_predictions > 0.3).astype(int), average='macro')
...
# 1. Train CheXNet (DenseNet-121 without CBAM)
class DenseNetNoCBAM(nn.Module):
    def __init__(self, num_classes=14):
        super(DenseNetNoCBAM, self).__init__()
        self.densenet = models.densenet121(pretrained=True)
        num_ftrs = self.densenet.classifier.in_features
        self.densenet.classifier = nn.Sequential(
            nn.Linear(num_ftrs, num_classes),
...
# Initialize and train CheXNet (DenseNet-121 without CBAM)
model_chexnet = DenseNetNoCBAM(num_classes=14).cuda()
optimizer = optim.SGD(model_chexnet.parameters(), lr=0.001, momentum=0.9)
train_model(
    model_chexnet,
    criterion,
    optimizer,
...
auc_chexnet, f1_chexnet = evaluate_model(model_chexnet, val_loader)
params_chexnet = count_parameters(model_chexnet)

ablation_results = pd.DataFrame({
    'Model Architecture': ['CheXNet (DenseNet-121)', 'CheXNet + CBAM'],
    'AUC': [auc_chexnet, auc_chexnet_cbam],
    'F1 Score': [f1_chexnet, f1_chexnet_cbam],
    'Parameters (Millions)': [params_chexnet / 1e6, params_chexnet_cbam / 1e6]
})
