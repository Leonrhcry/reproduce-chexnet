...
# Learnable kernel size for spatial attention
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        # Applying both attention mechanisms in parallel
        channel_attended = x * self.channel_attention(x)
        spatial_attended = x * self.spatial_attention(x)

        # Combine both attended features
        return (channel_attended + spatial_attended) / 2  # Combine with weighted sum
